# Apply only the verified Navigator 7020 PS7 configuration to the existing
# processing_system7_0 cell in the currently open block design.
# This script does NOT create any IP, reset block, ports, or connections.

set expected_part "xc7z020clg400-2"
set actual_part [get_property PART [current_project]]
if {$actual_part ne $expected_part} {
    error "Wrong project part: $actual_part (expected $expected_part)"
}

if {[current_bd_design -quiet] eq ""} {
    error "No block design is open. Open the system block design first."
}

set ps_cells [get_bd_cells -quiet processing_system7_0]
if {[llength $ps_cells] != 1} {
    error "Expected exactly one BD cell named processing_system7_0."
}

set ps_vlnv [get_property VLNV $ps_cells]
if {![string match "xilinx.com:ip:processing_system7:*" $ps_vlnv]} {
    error "processing_system7_0 is not a ZYNQ7 Processing System cell: $ps_vlnv"
}
...
save_bd_design
puts "PASS: Navigator 7020 PS configuration applied to processing_system7_0 only."
